            ...
            .child(
                div()
                    ...
                    .animotion_clip(
                        ...
                        |c| vec![
                            // 1. Highly Elastic / Underdamped Spring (Low damping ratio = lively oscillation)
                            BallProps {
                                x: 100.0,
                                y: c.prop(40.0)
                                    .spring(
                                        floor_y_offset,
                                        SpringParams::from_damping_ratio(220.0, 0.22),
                                    )
                                    .spring(
                                        40.0,
                                        SpringParams::from_damping_ratio(180.0, 0.60),
                                    )
                                    .clone(),
                                color: c.prop(hsla(0.75, 0.8, 0.50, 1.0)),
                            },
                            // 2. Balanced / Semi-damped Spring (Medium damping ratio = snappy UI bounce)
                            BallProps {
                                x: 260.0,
                                y: c.prop(80.0)
                                    .spring(
                                        floor_y_offset,
                                        SpringParams::from_damping_ratio(180.0, 0.45),
                                    )
                                    .spring(
                                        80.0,
                                        SpringParams::from_damping_ratio(180.0, 0.70),
                                    )
                                    .clone(),
                                color: c.prop(hsla(0.25, 0.8, 0.45, 1.0)),
                            },
                            // 3. Heavy / Critically Damped Spring (High damping ratio = smooth drop, minimal overshoot)
                            BallProps {
                                x: 420.0,
                                y: c.prop(20.0)
                                    .spring(
                                        floor_y_offset,
                                        SpringParams::from_damping_ratio(260.0, 0.82),
                                    )
                                    .spring(
                                        20.0,
                                        SpringParams::from_damping_ratio(200.0, 0.85),
                                    )
                                    .clone(),
...
fn main() {
    shared::run("Harmonic Spring Physics Marbles", |_, _| BouncingBallView);
}
